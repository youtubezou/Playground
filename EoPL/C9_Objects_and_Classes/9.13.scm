...
(define-datatype method-decl method-decl?
  (a-method-decl (method-name identifier?)
                 ...
                 (body expression?))
  (final-method-decl (method-name identifier?)
                 (vars (list-of identifier?))
                 (body expression?)))
...
(define-datatype method method?
  (a-method (final boolean?)
            (vars (list-of identifier?))
            (body expression?)
            (super-name identifier?)
            (field-names (list-of identifier?))))

(define (method->final m)
  (cases method m
    (a-method (final vars body super-name field-names) final)))
  ...
  (map (lambda (m-decl)
         (cases method-decl m-decl
           (a-method-decl (method-name vars body)
                          (list method-name
                                (a-method #f vars body super-name field-names)))
           (final-method-decl (method-name vars body)
                              (list method-name
                                    (a-method #t vars body super-name field-names)))))
       m-decls))

(define (check-override-final-rec super-m-env method-name)
  (if (null? super-m-env)
      #f
      (if (eqv? (caar super-m-env) method-name)
          (if (method->final (cadar super-m-env))
              (error 'final (cadar super-m-env))
              (check-override-final-rec (cdr super-m-env) method-name))
          (check-override-final-rec (cdr super-m-env) method-name))))

(define (check-override-final! super-m-env new-m-env)
  (if (null? new-m-env)
      #f
      (begin (check-override-final-rec super-m-env (caar new-m-env))
             (check-override-final! super-m-env (cdr new-m-env)))))

(define (merge-method-envs super-m-env new-m-env)
  (check-override-final! super-m-env new-m-env)
...
(define (value-of exp env)
  (cases expression exp
    ...
    (named-method-call-exp (class-name obj-exp method-name rands)
                           (let ((args (values-of-exps rands env))
                                 (obj (value-of obj-exp env)))
                             (apply-method (find-method class-name method-name)
                                           obj
                                           args)))
