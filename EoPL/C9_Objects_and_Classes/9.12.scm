...
(define grammar
  '((program ((arbno class-decls) expression) a-program)
    (class-decls ("class" identifier "extends" identifier
                  (arbno field-decl) (arbno method-decls)) a-class-decl)
    (field-decl ("field" identifier) public-field-decl)
    (field-decl ("pubilc-field" identifier) public-field-decl)
    (field-decl ("protected-field" identifier) protected-field-decl)
    (field-decl ("private-field" identifier) private-field-decl)
    (method-decls ("method" identifier "(" (separated-list identifier ",") ")" expression)
                  public-method-decl)
    (method-decls ("public-method" identifier "(" (separated-list identifier ",") ")" expression)
                  public-method-decl)
    (method-decls ("protected-method" identifier "(" (separated-list identifier ",") ")" expression)
                  protected-method-decl)
    (method-decls ("private-method" identifier "(" (separated-list identifier ",") ")" expression)
                  private-method-decl)
    ...
    (expression ("superfieldref" identifier) superfieldref-exp)
    (expression ("superfieldset" identifier "=" expression) superfieldset-exp)
...
(define-datatype expression expression?
  ...
  (method-call-exp (exp expression?)
                   (name identifier?)
                   (rands (list-of expression?)))
  (named-method-call-exp (class-name identifier?)
                         (exp expression?)
                         (name identifier?)
                         (rands (list-of expression?)))
  ...
  (fieldset-exp (obj-exp expression?)
                (name identifier?)
                (val-exp expression?))
  (superfieldref-exp (name identifier?))
  (superfieldset-exp (name identifier?)
                     (exp expression?))
  (self-exp))
...
(define-datatype field-decl field-decl?
  (public-field-decl (var identifier?))
  (protected-field-decl (var identifier?))
  (private-field-decl (var identifier?)))

(define (field-decl->name f)
  (cases field-decl f
    (public-field-decl (var) var)
    (protected-field-decl (var) var)
    (private-field-decl (var) var)))

; BEGIN: Method Declaration
(define-datatype method-decl method-decl?
  (public-method-decl (method-name identifier?)
                      (vars (list-of identifier?))
                      (body expression?))
  (protected-method-decl (method-name identifier?)
                         (vars (list-of identifier?))
                         (body expression?))
  (private-method-decl (method-name identifier?)
                       (vars (list-of identifier?))
                       (body expression?)))
...
(define (lookup-field-ref-rec names refs search-name)
  (if (null? names)
      (error 'lookup-field-ref search-name)
      (if (eqv? search-name (car names))
          (car refs)
          (lookup-field-ref-rec (cdr names) (cdr refs) search-name))))

(define (lookup-field-ref obj name)
  (let ((names (class->field-names (lookup-class (object->class-name obj))))
        (refs (object->fields obj)))
    (lookup-field-ref-rec names refs name)))
...
(define (lookup-super-field-ref obj super-name name)
  (let ((names (class->field-names (lookup-class super-name)))
        (refs (object->fields obj)))
    (lookup-field-ref-rec names refs name)))

(define (super-fieldref obj super-name name)
  (deref (lookup-super-field-ref obj super-name name)))

(define (super-fieldset obj super-name name val)
  (setref! (lookup-super-field-ref obj super-name name) val))

; BEGIN: Accessibility
(define-datatype access access?
  (public)
  (protected)
  (private))
...
(define (has-access? expect actual)
  (cases access expect
    (public () (public? actual))
    (protected () (or (public? actual)
                      (protected? actual)))
    (private () #t)))

; BEGIN: Method
(define-datatype method method?
  (a-method (access access?)
            (vars (list-of identifier?))
            (body expression?)
            (super-name identifier?)
            (field-names (list-of field-decl?))))

(define (method->access m)
  (cases method m
    (a-method (access vars body super-name field-names) access)))

(define (apply-method m self args)
  (cases method m
    (a-method (access vars body super-name field-names)
              (value-of body
                        (extend-env-vals
                         vars args
...
(define (method-decls->method-env m-decls super-name field-names)
  (map (lambda (m-decl)
         (cases method-decl m-decl
           (public-method-decl (method-name vars body)
                               (list method-name
                                     (a-method (public) vars body super-name field-names)))
           (protected-method-decl (method-name vars body)
                                  (list method-name
                                        (a-method (protected) vars body super-name field-names)))
           (private-method-decl (method-name vars body)
                                (list method-name
                                      (a-method (private) vars body super-name field-names)))))
       m-decls))
...
(define (extend-env-refs vars refs env)
  (if (null? vars)
      env
      (extend-env-refs (cdr vars) (cdr refs)
                       (extend-env-ref (field-decl->name (car vars)) (car refs) env))))
  ...
  (cases expression exp
    ...
    (method-call-exp (obj-exp method-name rands)
                     (let ((args (values-of-exps rands env))
                       ...
                       (let* ((method (find-method (object->class-name obj) method-name))
                              (actual (method->access method)))
                         (if (has-access? (public) actual)
                             (apply-method method obj args)
                             (error 'has-access? actual)))))
    ...
    (super-call-exp (method-name rands)
                    (let ((args (values-of-exps rands env))
                      ...
                      (let* ((method (find-method (apply-env-val env '%super) method-name))
                             (actual (method->access method)))
                        (if (has-access? (protected) actual)
                            (apply-method method obj args)
                            (error 'has-access? actual)))))
    ...
    (superfieldref-exp (name)
                       (super-fieldref (apply-env-val env '%self)
                                       (apply-env-val env '%super)
                                       name))
    (superfieldset-exp (name exp)
                       (let ((val (value-of exp env)))
                         (super-fieldset (apply-env-val env '%self)
                                         (apply-env-val env '%super)
                                         name val)))
    (self-exp ()
